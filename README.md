...
- `miniclaw/llm.py`：DeepSeek 模型配置层，基于 `pydantic-settings` 读取配置，并通过 `pydantic-ai` 的 OpenAI-compatible provider 创建模型实例。
...
项目通过 `DeepSeekClient` 接入 DeepSeek 的 OpenAI-compatible API。运行时使用 `pydantic-settings` 自动读取项目根目录 `.env` 中的配置：
...
智能体运行循环和工具调用由 `pydantic-ai` 接管，DeepSeek 仍然负责模型推理与 function calling。
